...
$(document).ready(function () {
    ...
    //FormatNumericValues();//MDC - Not in use
});
...
function GetfilterProperty(mapData) {
   ...
   // document.getElementById("divLoader").style.visibility = "visible";
    ...
    customStartDate = searchParams.get('date-range1') != null ? searchParams.get('date-range1') : $("#date-range1").val();
    if (customStartDate)
        $("#date-range1").val(customStartDate);
    customEndDate = searchParams.get('date-range2') != null ? searchParams.get('date-range2') : $("#date-range2").val();
    if (customEndDate)
        $("#date-range2").val(customEndDate)
    ...
    //========================================================================
    var propertKeyword = searchParams.get('txtSearchProperty') != null ? searchParams.get('txtSearchProperty') : $("#txtSearchProperty").val();
    if (propertKeyword)
        $("#txtSearchProperty").val(propertKeyword);
    ...
    if (searchParams.get('txtSelectAll2'))
        $("#txtSelectAll2").attr('checked', 'checked');
    if (searchParams.get('txtMultiFamily'))
        $("#txtMultiFamily").attr('checked', 'checked');
    if (searchParams.get('txthouseicon'))
        $("#txthouseicon").attr('checked', 'checked');
    if (searchParams.get('txtoffice'))
        $("#txtoffice").attr('checked', 'checked');
    if (searchParams.get('txtIndustrial'))
        $("#txtIndustrial").attr('checked', 'checked');
    if (searchParams.get('txtretail'))
        $("#txtretail").attr('checked', 'checked');
    if (searchParams.get('txtshopping'))
        $("#txtshopping").attr('checked', 'checked');
    if (searchParams.get('txtSpeciality'))
        $("#txtSpeciality").attr('checked', 'checked');
    if (searchParams.get('txtHealth'))
        $("#txtHealth").attr('checked', 'checked');
    if (searchParams.get('txtHospitality'))
        $("#txtHospitality").attr('checked', 'checked');
    if (searchParams.get('txtSports'))
        $("#txtSports").attr('checked', 'checked');
    if (searchParams.get('txtLand'))
        $("#txtLand").attr('checked', 'checked');
    if (searchParams.get('txtresidentialportfolio'))
        $("#txtresidentialportfolio").attr('checked', 'checked');


        //Terms dropdown list
    if (searchParams.get('terms1'))
        $("#terms1").attr('checked', 'checked');
    if (searchParams.get('terms2'))
        $("#terms2").attr('checked', 'checked');
    if (searchParams.get('terms3'))
        $("#terms3").attr('checked', 'checked');
    if (searchParams.get('terms4'))
        $("#terms4").attr('checked', 'checked');
    if (searchParams.get('terms5'))
        $("#terms5").attr('checked', 'checked');
    if (searchParams.get('terms6'))
        $("#terms6").attr('checked', 'checked');
    if (searchParams.get('terms7'))
        $("#terms7").attr('checked', 'checked');
    if (searchParams.get('terms8'))
        $("#terms8").attr('checked', 'checked');
    ...
    var PropertyListSelectedValue = filterCheckboxvalues;//propertyTypeName != null ? propertyTypeName : filterCheckboxvalues;//


    var Af_cityName = searchParams.get('marketName') != null ? searchParams.get('marketName') : $("#marketName").val();
    if (Af_cityName)
        $("marketName").val(Af_cityName);

    var Af_zipCode = searchParams.get('zipName') != null ? searchParams.get('zipName') : $("#zipName").val();
    if (Af_zipCode)
        $("zipName").val(Af_zipCode);

    var Af_LandMinAcres = searchParams.get('Af_LandMinAcres') != null ? searchParams.get('Af_LandMinAcres') : $("#Af_LandMinAcres").val(); 
    if (Af_LandMinAcres)
        $("#Af_LandMinAcres").val(Af_LandMinAcres);

    var Af_LandMaxAcres = searchParams.get('Af_LandMaxAcres') != null ? searchParams.get('Af_LandMaxAcres') : $("#Af_LandMaxAcres").val(); 
    if (Af_LandMaxAcres)
        $("#Af_LandMaxAcres").val(Af_LandMaxAcres);

    var Af_UnitBedsMin = searchParams.get('Af_UnitBedsMin') != null ? searchParams.get('Af_UnitBedsMin') : $("#Af_UnitBedsMin").val(); 
    if (Af_UnitBedsMin)
        $("#Af_UnitBedsMin").val(Af_UnitBedsMin);

    var Af_UnitBedsMax = searchParams.get('Af_UnitBedsMax') != null ? searchParams.get('Af_UnitBedsMax') : $("#Af_UnitBedsMax").val(); 
    if (Af_UnitBedsMax)
        $("#Af_UnitBedsMax").val(Af_UnitBedsMax);

    var Af_YearBuiltMin = searchParams.get('Af_YearBuiltMin') != null ? searchParams.get('Af_YearBuiltMin') : $("#Af_YearBuiltMin").val(); 
    if (Af_YearBuiltMin)
        $("#Af_YearBuiltMin").val(Af_YearBuiltMin);

    var Af_YearBuiltMax = searchParams.get('Af_YearBuiltMax') != null ? searchParams.get('Af_YearBuiltMax') : $("#Af_YearBuiltMax").val(); 
    if (Af_YearBuiltMax)
        $("#Af_YearBuiltMax").val(Af_YearBuiltMax);

    var Af_Keyword = searchParams.get('keywordId') != null ? searchParams.get('keywordId') : $("#keywordId").val(); 
    if (Af_Keyword)
        $("#keywordId").val(Af_Keyword);

    var Af_EnteredDate = searchParams.get('dateId') != null ? searchParams.get('dateId') : $("#dateId").val(); 
    if (Af_EnteredDate)
        $("#dateId").val(Af_EnteredDate);

    var Af_reidyId = searchParams.get('reidyId') != null ? searchParams.get('reidyId') : $("#reidyId").val(); 
    if (Af_reidyId)
        $("#reidyId").val(Af_reidyId);

    var Region = searchParams.get('regionName') != null ? searchParams.get('regionName') : $("#regionName").val(); 
    if (Region)
        $("#regionName").val(Region);

    var ReidyId = searchParams.get('reidyId') != null ? searchParams.get('reidyId') : $("#reidyId").val(); 
    if (ReidyId)
        $("#reidyId").val(ReidyId);


    //alert(searchParams.get('inlineRadioOptions'));


    switch (searchParams.get('inlineRadioOptions')) {
        case '1':
            $("#sfFieldradio").prop('checked', false);
            $("#acrefield").prop('checked', false);
            $("#unitRadio").prop('checked', false);
            $("#totalradio").prop("checked", true).trigger("click");
            break;
        case '2':
            $("#totalradio").prop('checked', false);
            $("#acrefield").prop('checked', false);
            $("#unitRadio").prop('checked', false);
            $("#sfFieldradio").prop("checked", true).trigger("click");
            break;
        case '3':
            $("#totalradio").prop('checked', false);
            $("#unitRadio").prop('checked', false);
            $("#sfFieldradio").prop('checked', false);
            $("#acrefield").prop("checked", true).trigger("click");
            break;
        case '4':
            $("#totalradio").prop('checked', false);
            $("#sfFieldradio").prop('checked', false);
            $("#acrefield").prop('checked', false);
            $("#unitRadio").prop("checked", true).trigger("click");
            break;
        default:
            // code block   $("#radio_1").prop("checked", true).trigger("click");
            //alert("Failed!!");
    }
    ...
    var UnitValue = UCategory;//UCategory1 != null ? UCategory1 : UCategory;//



    var Af_minPrice = searchParams.get('Af_minPrice') != null ? searchParams.get('Af_minPrice') : $("#Af_minPrice").val(); 
    if (Af_minPrice)
        $('#Af_minPrice').val(Af_minPrice);

    var minTargetPrice1 = searchParams.get('MinTargetPrice') != null ? searchParams.get('MinTargetPrice') : $("#MinTargetPrice").val(); 
    if (minTargetPrice1)
        $("#MinTargetPrice").val(minTargetPrice1);

    var minPriceValue = minTargetPrice1 != null ? minTargetPrice1 : Af_minPrice;
    Af_minPrice = minPriceValue.replace(/,/g, '');


    var Af_maxPrice = searchParams.get('Af_maxPrice') != null ? searchParams.get('Af_maxPrice') : $("#Af_maxPrice").val(); 
    if (Af_maxPrice)
        $('#Af_maxPrice').val(Af_maxPrice);

    var maxTargetPrice1 = searchParams.get('MaxTargetPrice') != null ? searchParams.get('MaxTargetPrice') : $("#MaxTargetPrice").val(); 
    if (maxTargetPrice1)
        $("#MaxTargetPrice").val(maxTargetPrice1);

    var maxPriceValue = maxTargetPrice1 != null ? maxTargetPrice1 : Af_maxPrice;
    Af_maxPrice = maxPriceValue.replace(/,/g, '');


    var Af_BuildingMinSF = searchParams.get('Af_BuildingMinSF') != null ? searchParams.get('Af_BuildingMinSF') : $("#Af_BuildingMinSF").val(); 
    if (Af_BuildingMinSF)
        $("#Af_BuildingMinSF").val(Af_BuildingMinSF);

    var BuiltMinSF = searchParams.get('txtMinBuildSF') != null ? searchParams.get('txtMinBuildSF') : $("#txtMinBuildSF").val(); 
    if (BuiltMinSF)
        $("#txtMinBuildSF").val(BuiltMinSF);
    ...
    var Af_BuildingMaxSF = searchParams.get('Af_BuildingMaxSF') != null ? searchParams.get('Af_BuildingMaxSF') : $("#Af_BuildingMaxSF").val(); 
    if (Af_BuildingMaxSF)
        $("#Af_BuildingMaxSF").val(Af_BuildingMaxSF);

    var BuiltMaxSF = searchParams.get('txtMaxBuildSF') != null ? searchParams.get('txtMaxBuildSF') : $("#txtMaxBuildSF").val(); 
    if (BuiltMaxSF)
        $("#txtMaxBuildSF").val(BuiltMaxSF);
    ...
    //Save DueDiligence Checks
    if (searchParams.get('txtAppraisal'))
        $("#txtAppraisal").attr('checked', 'checked');
    if (searchParams.get('txtEnvironmental'))
        $("#txtEnvironmental").attr('checked', 'checked');
    if (searchParams.get('txtSurvey'))
        $("#txtSurvey").attr('checked', 'checked');
    if (searchParams.get('txtInspection'))
        $("#txtInspection").attr('checked', 'checked');
    ...
    var fromfundToClose = searchParams.get('txtminFtC') != null ? searchParams.get('txtminFtC') : $("#txtminFtC").val(); 
    if (fromfundToClose)
        $("#txtminFtC").val(fromfundToClose);

    var tofundToClose = searchParams.get('txtmaxFtC') != null ? searchParams.get('txtmaxFtC') : $("#txtmaxFtC").val(); 
    if (tofundToClose)
        $("#txtmaxFtC").val(tofundToClose);

    var interestMin = searchParams.get('interestMin') != null ? searchParams.get('interestMin') : $("#interestMin").val();
    if (interestMin)
        $("#interestMin").val(interestMin);

    var interestMax = searchParams.get('interestMax') != null ? searchParams.get('interestMax') : $("#interestMax").val();
    if (interestMax)
        $("#interestMax").val(interestMax);



    var selectprograms = searchParams.get('selectprograms') != null ? searchParams.get('selectprograms') : $("#selectprograms").val();
    if (selectprograms)
        $("#selectprograms").val(selectprograms);
    ...
    var propFilterModel = {
        ...
        IsLandAcre: false,
        IsLandSF: false,
        IsLandHectares: false,
        IsLandMS: false
    };


    var strPropertyFilterModel = JSON.stringify({ 'PropertyFilterModel': propFilterModel });


    var myPropertyFilterModel = '{"FromfundToClose":';//string
    if (!fromfundToClose)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + fromfundToClose + '",'

    myPropertyFilterModel += '"TofundToClose":';//string
    if (!tofundToClose)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + tofundToClose + '",'

    myPropertyFilterModel += '"PropertKeyword":';//string
    if (!propertKeyword)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + propertKeyword + '",'

    myPropertyFilterModel += '"City":';//string
    if (!Af_cityName)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + Af_cityName + '",'

    myPropertyFilterModel += '"ZipCode":';//int
    if (!Af_zipCode)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += Af_zipCode + ','

    myPropertyFilterModel += '"MinPrice":';//string
    if (!Af_minPrice)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + Af_minPrice + '",'

    myPropertyFilterModel += '"MaxPrice":';//string
    if (!Af_maxPrice)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + Af_maxPrice + '",'

    myPropertyFilterModel += '"BuildingMinSF":';//decimal
    if (!Af_BuildingMinSF)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += Af_BuildingMinSF + ','

    myPropertyFilterModel += '"BuildingMaxSF":';//decimal
    if (!Af_BuildingMaxSF)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += Af_BuildingMaxSF + ','

    myPropertyFilterModel += '"LandMinAcres":';//decimal
    if (!Af_LandMinAcres)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += Af_LandMinAcres + ','

    myPropertyFilterModel += '"LandMaxAcres":';//decimal
    if (!Af_LandMaxAcres)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += Af_LandMaxAcres + ','

    myPropertyFilterModel += '"UnitBedsMin":';//int
    if (!Af_UnitBedsMin)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += Af_UnitBedsMin + ','

    myPropertyFilterModel += '"UnitBedsMax":';//int
    if (!Af_UnitBedsMax)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += Af_UnitBedsMax + ','

    myPropertyFilterModel += '"YearBuiltMin":';//int
    if (!Af_YearBuiltMin)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += Af_YearBuiltMin + ','

    myPropertyFilterModel += '"YearBuiltMax":';//int
    if (!Af_YearBuiltMax)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += Af_YearBuiltMax + ','

    myPropertyFilterModel += '"Keyword":';//string
    if (!Af_Keyword)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + Af_Keyword + '",'

    myPropertyFilterModel += '"EnteredDate":';//string
    if (!Af_EnteredDate)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + Af_EnteredDate + '",'

    myPropertyFilterModel += '"StartDate":';//string
    if (!customStartDate)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + customStartDate + '",'

    myPropertyFilterModel += '"EndDate":';//string
    if (!customEndDate)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + customEndDate + '",'

    myPropertyFilterModel += '"ReidyId":';//int
    if (!Af_reidyId)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += Af_reidyId + ','

    myPropertyFilterModel += '"PropertyTypeList":';//string
    if (!PropertyListSelectedValue)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + PropertyListSelectedValue + '",'

    myPropertyFilterModel += '"RegionName":';//string
    if (!Region)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + Region + '",'

    //myPropertyFilterModel += '"ReidyId":';//int  //Second ReidyId ???
    //if (!ReidyId)
    //    myPropertyFilterModel += '0,'
    //else
    //    myPropertyFilterModel += ReidyId + ','

    myPropertyFilterModel += '"UnitCategory":';//int
    if (!UnitValue)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += UnitValue + ','

    myPropertyFilterModel += '"DueDiligenceValue":';//string
    if (!DiliganceInputValue)
        myPropertyFilterModel += 'null,'
    else
        myPropertyFilterModel += '"' + DiliganceInputValue + '",'

    myPropertyFilterModel += '"MinLat":';//decimal
    if (!MinLat)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += MinLat + ','

    myPropertyFilterModel += '"MinLng":';//decimal
    if (!MinLng)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += MinLng + ','

    myPropertyFilterModel += '"MaxLat":';//decimal
    if (!MaxLat)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += MaxLat + ','

    myPropertyFilterModel += '"MaxLng":';//decimal
    if (!MaxLng)
        myPropertyFilterModel += '0,'
    else
        myPropertyFilterModel += MaxLng + ','

    myPropertyFilterModel += '"UserId":null,"Status":0,"MaxYearBuilt":null,"MinYearBuilt":null,"BuiltMaxSF":null,"BuiltMinSF":null,"SearchName":null,"StateId":0,"IsAppraisal":false,"IsSurvey":false,"IsEnvironmental":false,"IsInspection":false,"IsTotalPrice":false,"IsPriceSF":false,"IsPriceAcr":false,"IsPriceUnit":false,"IsBuildingSF":false,"IsBuildingSM":false,"IsLandAcre":false,"IsLandSF":false,"IsLandHectares":false,"IsLandMS":false}';
        //

    console.log(myPropertyFilterModel);
...
function Allclear() {
    ...
    $('#terms5').prop('checked', false);
    $('#terms6').prop('checked', false);
    $('#terms7').prop('checked', false);
    $('#terms8').prop('checked', false);

    $('#txtAppraisal').prop('checked', false);
    $('#txtSurvey').prop('checked', false);
    $('#txtEnvironmental').prop('checked', false);
    $('#txtInspection').prop('checked', false);
