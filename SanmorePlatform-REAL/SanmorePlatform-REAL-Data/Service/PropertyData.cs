    ...
    public class PropertyData : IPropertyData
    {
        ...
        public async Task<List<Sp_GetPropertyListModel>> GetPropertyListAsync(PropertyFilterModel filterModel)
        {
            try
            {
                ...
                //int
                var myStatus = new SqlParameter("@status", (object?)filterModel.Status ?? DBNull.Value);//0//DBNull.Value
                //nvarchar(MAX)
                var myCity = new SqlParameter("@City", (object?)filterModel.City ?? DBNull.Value);
                //nvarchar(MAX)
                var myMaxYearBuilt = new SqlParameter("@maxYearBuilt", (object?)filterModel.MaxYearBuilt ?? DBNull.Value);
                //nvarchar(MAX)
                var myMinYearBuilt = new SqlParameter("@minYearBuilt", (object?)filterModel.MinYearBuilt ?? DBNull.Value);
                //decimal(18,2)
                var myFromfundToClose = new SqlParameter("@FromfundToClose", (object?)filterModel.FromfundToClose ?? DBNull.Value);
                //decimal(18,2)
                var myTofundToClose = new SqlParameter("@TofundToClose", (object?)filterModel.TofundToClose ?? DBNull.Value);
                //int
                var myZipCode = new SqlParameter("@ZipCode", (object?)filterModel.ZipCode ?? 0);//DBNull.Value
                //decimal(18,2)
                var myBuildingMinSF = new SqlParameter("@BuildingMinSF", (object?)filterModel.BuildingMinSF ?? DBNull.Value);
                //decimal(18,2)
                var myBuildingMaxSF = new SqlParameter("@BuildingMaxSF", (object?)filterModel.BuildingMaxSF ?? DBNull.Value);
                //decimal(18,2)
                var myLandMinAcres = new SqlParameter("@LandMinAcres", (object?)filterModel.LandMinAcres ?? DBNull.Value);
                //decimal(18,2)
                var myLandMaxAcres = new SqlParameter("@LandMaxAcres", (object?)filterModel.LandMaxAcres ?? DBNull.Value);
                //int
                var myUnitBedsMin = new SqlParameter("@UnitBedsMin", (object?)filterModel.UnitBedsMin ?? 0);//DBNull.Value
                //int
                var myUnitBedsMax = new SqlParameter("@UnitBedsMax", (object?)filterModel.UnitBedsMax ?? 0);//DBNull.Value
                //nvarchar(MAX)
                var myKeyword = new SqlParameter("@Keyword", (object?)filterModel.Keyword ?? DBNull.Value);
                //nvarchar(MAX)
                var myEnteredDate = new SqlParameter("@EnteredDate", (object?)filterModel.EnteredDate ?? DBNull.Value);
                //int
                var myReidyId = new SqlParameter("@ReidyId", (object?)filterModel.ReidyId ?? DBNull.Value);
                //int
                var myYearBuiltMin = new SqlParameter("@YearBuiltMin", (object?)filterModel.YearBuiltMin ?? DBNull.Value);
                //int
                var myYearBuiltMax = new SqlParameter("@YearBuiltMax", (object?)filterModel.YearBuiltMax ?? DBNull.Value);
                //varchar(MAX)
                var myPropertyTypeList = new SqlParameter("@PropertyTypeList", (object?)filterModel.PropertyTypeList ?? DBNull.Value);
                //varchar(MAX)
                var myPropertyKeyword = new SqlParameter("@PropertyKeyword", (object?)filterModel.PropertKeyword ?? DBNull.Value);
                //varchar(MAX)
                var myRegionName = new SqlParameter("@RegionName", (object?)filterModel.RegionName ?? DBNull.Value);
                //varchar(MAX)
                var myFromDate = new SqlParameter("@FromDate", (object?)filterModel.StartDate ?? DBNull.Value);
                //varchar(MAX)
                var myToDate = new SqlParameter("@ToDate", (object?)filterModel.EndDate ?? DBNull.Value);


                //Connecting to Database Table: Property 
                var mySP = _context.Sp_GetPropertyList.FromSqlRaw($"exec Sp_GetPropertyList @status, @City,@maxYearBuilt,@minYearBuilt,@FromfundToClose,@TofundToClose,@ZipCode,@BuildingMinSF,@BuildingMaxSF,@LandMinAcres,@LandMaxAcres,@UnitBedsMin,@UnitBedsMax,@Keyword,@EnteredDate,@ReidyId,@YearBuiltMin,@YearBuiltMax,@PropertyTypeList,@PropertyKeyword,@RegionName,@FromDate,@ToDate",
                    myStatus, myCity, myMaxYearBuilt, myMinYearBuilt, myFromfundToClose, myTofundToClose, myZipCode, myBuildingMinSF, myBuildingMaxSF, myLandMinAcres, myLandMaxAcres, myUnitBedsMin, myUnitBedsMax, myKeyword, myEnteredDate, myReidyId, myYearBuiltMin, myYearBuiltMax, myPropertyTypeList, myPropertyKeyword, myRegionName, myFromDate, myToDate);//.ToListAsync();

                var AllReports = (await mySP.ToListAsync());
                ...
                if (AllReports.Count != 0)
                {
                    ...
                    {
                        name = _context.PropertyImage.Where(q => q.PropertyId == report.PropertyId).Select(x => x.ImageName).FirstOrDefault();
                        ...
                        if (name == null || name == "")
                        {
                            report.ImageName = "no_imge_1.jpg";
                        }
